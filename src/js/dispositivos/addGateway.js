App = {
  ...
  render: function() {
    ...
    sensorReg.replaceWith(
      ...
      "<br>" +
      "<label for='deveui'>devEui (bytes8):</label>" +
      "<input type='text' id='deveui' maxlength='12' required>" +
      "<br>" +
  ...
  sensorRegistration: function(){
    ...
    App.contracts.Protocol.deployed().then(function(instance) {
      ...
      instance.gatewayRegistration(idG,devEui,mac,pubKeyL,pubKeyR, { from: App.account });

    }).catch(function(error) {
      console.warn(error);
      alert("Ha habido un error con el proceso de registro del gateway.");
    });
