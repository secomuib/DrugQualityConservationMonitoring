App = {
  ...
  render: function() {
    ...
    addNotif.replaceWith(
      ...
      "<br>" +
      "<input type='button' id='submitButton' value='Registrar warning' onclick='App.warningRegistration()'>" +
      "</form>"
    );
  ...
  warningRegistration: function(){
    ...
    App.contracts.Protocol.deployed().then(async function(instance) {
      ...
      try{
        await instance.newWarning(App.account, idS, warn, dur, aver, maxtemp, mintemp, { from: App.account });
      } catch (e){
        console.warn(e.message);
        auxE = e.message;
        if (auxE.includes('El sensor no dispone de certificado activo.')){
          alert("No existe un certificado valido del sensor al que se desea asociar la notificación.")
        }
